...
import { Building2, Plus, BarChart3, BookOpen, MessageSquare, User, DollarSign, TrendingUp, CheckCircle2, Clock, X } from "lucide-react";
...
const HostDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (userRole === "student") { navigate("/dashboard"); return; }
    fetchData();
  }, [user, userRole]);
  ...
  const fetchData = async () => {
    ...
    // Filter bookings for host's properties
  ...
  const handleAddProperty = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("properties").insert({
      ...
      gender_restriction: form.gender_restriction as any,
            ...
            <div className="bg-card rounded-2xl border shadow-elevated max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
              ...
              <form onSubmit={handleAddProperty} className="space-y-4">
                <div>
                  ...
                  <Input value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="e.g. Sunrise Hostel — Female Only" className="mt-1" />
                </div>
                <div>
                  <Label>Description</Label>
                  <Textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Describe your property..." className="mt-1" rows={3} />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Input type="number" value={form.total_rooms} onChange={e => setForm({...form, total_rooms: e.target.value})} className="mt-1" min="1" />
                  </div>
                </div>
                <div>
                  <Label>Distance to Campus Gate (meters)</Label>
                  <Input type="number" value={form.distance_to_gate_meters} onChange={e => setForm({...form, distance_to_gate_meters: e.target.value})} placeholder="e.g. 250" className="mt-1" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Textarea value={form.house_rules} onChange={e => setForm({...form, house_rules: e.target.value})} placeholder="Any rules for tenants..." className="mt-1" rows={2} />
                </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                      ...
                      <div className="text-right">
                        ...
                        <Link to={`/listing/${p.id}`}>
                          <Button variant="ghost" size="sm" className="mt-2">View Listing</Button>
                        </Link>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="text-right space-y-2">
                        ...
                        {b.status === "PENDING" && (
                          <div className="flex gap-2 justify-end">
                            ...
                            <Button size="sm" variant="destructive" onClick={() => handleBookingAction(b.id, "CANCELLED")}>
                              Decline
                            </Button>
                          </div>
                        )}
          ...
          <TabsContent value="earnings">
            <div className="bg-card rounded-xl border p-6">
              ...
              <p className="text-xs text-muted-foreground mt-6 text-center">
                Payouts are released to your bank account after student confirms move-in (minus 8% platform commission)
              </p>
            </div>
          </TabsContent>
