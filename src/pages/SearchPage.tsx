...
import { MapPin, ArrowLeft, Map, List, SlidersHorizontal } from "lucide-react";
...
const amenityList = ["WiFi", "Generator", "Borehole", "CCTV", "Parking", "Security Guard", "Reading Room", "AC", "Furnished", "Laundry Area"];
...
const SearchPage = () => {
  ...
  const fetchProperties = async () => {
    ...
    let query = supabase.from("properties").select(`
      *, universities(name, short_name, latitude, longitude),
      rooms(room_type, price_per_session, price_per_month, available_count),
      reviews:reviews(rating)
    `).eq("is_active", true);

    if (selectedUni) query = query.eq("universities.short_name", selectedUni);
    ...
    if (!error && data) {
      ...
      if (enriched.length > 0 && enriched[0].universities?.latitude) {
        setUniCenter([enriched[0].universities.latitude, enriched[0].universities.longitude]);
      }
    }
    setLoading(false);
  };
  ...
  if (genderFilter) filtered = filtered.filter(p => p.gender_restriction === genderFilter);
      ...
      <main className="flex-1 container mx-auto px-4 py-6">
        ...
        {showFilters && (
          <div className="bg-card rounded-xl border p-5 mb-6 space-y-5 animate-fade-in">
            ...
            <Button variant="ghost" size="sm" onClick={() => { setTypeFilter(""); setGenderFilter(""); setAmenityFilters([]); setPriceRange([0, 1000000]); setMaxDistance(5000); }}>
              Clear All Filters
            </Button>
          </div>
        )}
